    ...
    value: function render() {
      ...
      if (position && (0, _DataUtils.isNumber)(position.x) && (0, _DataUtils.isNumber)(position.y)) {
        ...
        if (boxWidth > 0 && boxHeight > 0 && coordinate) {
          translateX = function () {
            if (position && (0, _DataUtils.isNumber)(position.x)) {
              return position.x;
            }

            if (placement === 'left') {
              return Math.max(coordinate.x - boxWidth - offset > viewBox.x ? coordinate.x - boxWidth - offset : coordinate.x + offset, viewBox.x);
            } else {
              return Math.max(coordinate.x + boxWidth + offset > viewBox.x + viewBox.width ? coordinate.x - boxWidth - offset : coordinate.x + offset, viewBox.x);
            }
          }();
